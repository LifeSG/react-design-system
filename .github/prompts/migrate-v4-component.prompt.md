...
-   Create functional tests for the component using the existing Playwright setup.
...
-   Declare a component story for the functional tests to run against. The story should be added to `e2e/nextjs-app/components/<component>/<story>.e2e.tsx` and should include the necessary props and state to demonstrate the component's functionality. Each story should test 1 specific scenario or use case.
...
Pause and allow the user to verify the test specs before proceeding to the next step.
...
-   For props used as CSS values, they can be converted to CSS variables and passed down to the `styled` component as inline styles. The CSS variable name should be formatted as `--fds-internal-componentName-subComponent-propertyType`. For example:
    ...
    const MyComponent = ({ backgroundColor, ...rest }) => {
      ...
      useApplyStyle(containerRef, tokens.container.backgroundColor, backgroundColor);
