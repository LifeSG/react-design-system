...
`,qe=({external:t=!1,children:r,...a})=>e(Je,{...a,children:[r,t&&n(Ke,{})]}),Qe=o.div`
    ...
    position: relative;
    align-items: center;
    border: 1px solid ${Ce.Neutral[5]};
    border-radius: 4px;
    background: ${Ce.Neutral[8]};
    height: 3rem;
    width: 100%;
    ...
    :focus,
    :focus-within {
        border: 1px solid ${Ce.Accent.Light[1]};
        box-shadow: ${ze};
    }
    ...
    ${t=>t.$readOnly?l`
            ...
            `:t.disabled?l`
                background: ${Ce.Neutral[6](t)};
                :hover {
                    cursor: not-allowed;
                }

                :focus-within {
                    border: 1px solid ${Ce.Neutral[5](t)};
                    box-shadow: none;
                }
            `:t.$error?l`
                border: 1px solid ${Ce.Validation.Red.Border(t)};
...
`;o(an)`
    ${t=>{if(t.$compress)return l`
            ...
            `}}
`;const on=({disabled:t,error:o,value:l,onChange:c,onBlur:u,onChangeRaw:s,onBlurRaw:h,readOnly:d,id:f,...g})=>{const[p,S]=r(""),[$,v]=r(""),[F,D]=r(""),[B,H]=r("none"),b=a(null),w=a(null),_=a(null),x=a(null),O=a(p),M=a($),C=a(F),k=a(B),E=t=>{O.current=t,S(t)},z=t=>{M.current=t,v(t)},A=t=>{C.current=t,D(t)},W=t=>{k.current=t,H(t)};i((()=>(document.addEventListener("mousedown",T),b.current&&b.current.addEventListener("keydown",V),()=>{document.removeEventListener("mousedown",T),b.current&&b.current.removeEventListener("keydown",V)})),[B]),i((()=>{"day"===B&&2===p.length&&_.current&&_.current.focus()}),[p]),i((()=>{"month"===B&&2===$.length&&x.current&&x.current.focus()}),[$]),i((()=>{Y(l)}),[l]);const T=e=>{t||d||b&&b.current.contains(e.target)||"none"!==k.current&&(W("none"),P())},V=t=>{"year"===t.target.name&&"Tab"===t.code&&(W("none"),P())},N=t=>{W(t.target.name),t.target.select()},I=t=>{const e=t.target.name,n=m.padValue(t.target.value,!0);switch(e){case"day":E(n);break;case"month":z(n)}const r=p.length&&$.length&&4===F.length,a="day"===e,i=y.clampMonth($);r&&(E(m.padValue(y.clampDay(a?n:p,i,F))),z(m.padValue(i)))},L=t=>{const e=t.target.name,n=t.target.value.replace(/[^0-9]/g,"");switch(e){case"day":E(n),R(n,e);break;case"month":z(n),R(n,e);break;case"year":A(n),R(n,e)}},j=t=>{"Backspace"!==t.code&&"Backspace"!==t.key||("month"===B&&0===$.length&&w.current.focus(),"year"===B&&0===F.length&&_.current.focus())},Y=t=>{if(void 0===t||""===t)E(""),z(""),A("");else{const e=new Date(t);if(!isNaN(e.getTime())){const t=(e.getMonth()+1).toString(),n=e.getFullYear().toString(),r=y.clampDay(e.getDate().toString(),t,n);E(m.padValue(r)),z(m.padValue(t)),A(n)}}},R=(t,e)=>{if(c){const n={day:p,month:$,year:F};n[e]=t;const r=X(n);c(r)}if(s){const n=[..."day"===e?[m.padValue(t)]:[p],..."month"===e?[m.padValue(t)]:[$],..."year"===e?[t]:[F]];s(n)}},P=()=>{if(u){const t={day:O.current,month:M.current,year:C.current},e=X(t);u(e)}if(h){const t=[m.padValue(O.current.toString()),m.padValue(M.current.toString()),C.current];h(t)}},X=t=>{const e=[t.year,t.month,t.day];return t.day.length>=1&&t.month.length>=1&&4===t.year.length?e.join("-"):e.every((t=>""===t))?"":ln};return n(Qe,{ref:b,onClick:()=>{"none"===B&&w.current&&w.current.focus()},disabled:t,$error:o,id:f,"data-testid":g["data-testid"],$readOnly:d,children:e(tn,{$readOnly:d,children:[n(en,{name:"day",maxLength:2,value:p,ref:w,onFocus:N,onBlur:I,onChange:L,disabled:t,type:"text",inputMode:"numeric",pattern:"[0-9]{2}","data-testid":"day-input","aria-label":"day-input",readOnly:d,placeholder:"day"!==B||d?"DD":""}),n(an,{$hide:0===p.length,children:"/"}),n(nn,{name:"month",maxLength:2,value:$,ref:_,onFocus:N,onBlur:I,onChange:L,onKeyDown:j,disabled:t,type:"text",inputMode:"numeric",pattern:"[0-9]{2}","data-testid":"month-input","aria-label":"month-input",readOnly:d,placeholder:"month"!==B||d?"MM":""}),n(an,{$hide:0===$.length,children:"/"}),n(rn,{name:"year",maxLength:4,value:F,ref:x,onFocus:N,onBlur:I,onChange:L,onKeyDown:j,disabled:t,type:"text",inputMode:"numeric",pattern:"[0-9]{4}","data-testid":"year-input","aria-label":"year-input",readOnly:d,placeholder:"year"!==B||d?"YYYY":""})]})})},ln="Invalid date";export{on as DateInput};
