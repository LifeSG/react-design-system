...
type ButtonStyleType = "default" | "secondary" | "light" | "link";
type ButtonSizeType = "large" | "default" | "small";

const STYLE_MAP: Record<string, string> = {
    default: button.styleDefault,
    secondary: button.styleSecondary,
    light: button.styleLight,
    link: button.styleLink,
};

const STYLE_DANGER_MAP: Record<string, string> = {
    default: button.styleDefaultDanger,
    secondary: button.styleSecondaryDanger,
    light: button.styleLightDanger,
    link: button.styleLinkDanger,
};

const SIZE_MAP: Record<string, string> = {
    large: button.sizeLarge,
    default: button.sizeDefault,
    small: button.sizeSmall,
};

const SIZE_ICON_ONLY_MAP: Record<string, string> = {
    large: button.sizeLargeIconOnly,
    default: button.sizeDefaultIconOnly,
    small: button.sizeSmallIconOnly,
};
...
export class Button extends LitElement {
    ...
    render() {
        ...
        const showDisabledStyle = this.disabled;
        ...
        if (showDisabledStyle) {
            classes.push(button.styleDisabled);
        } else if (this.danger) {
            classes.push(
                STYLE_DANGER_MAP[this.styleType] ?? button.styleDefaultDanger
            );
        } else {
            classes.push(STYLE_MAP[this.styleType] ?? button.styleDefault);
        }

        // Size variant
        if (this._iconOnly) {
            classes.push(
                SIZE_ICON_ONLY_MAP[this.sizeType] ?? button.sizeDefaultIconOnly
            );
        } else {
            classes.push(SIZE_MAP[this.sizeType] ?? button.sizeDefault);
        }
        ...
        // Min-width when has text
        if (!this._iconOnly) {
            classes.push(button.hasMinWidth);
        }
