...
> **⚠ When using v3 docs as a fallback:**
>
> -   **Do not copy styled-components patterns** — v4 uses CSS Modules. Any `styled-components` usage, theme props (`p.theme.color.*`), or `StyleSheetManager` from v3 examples will fail.
> -   **v3 design tokens do not exist in v4** — `Color`, `FontSize`, `TextStyleUtils`, etc. are not exported. Use `var(--fds-colour-*)` / `var(--fds-font-*)` CSS variables or the `Colour`/`Spacing` helpers from `@lifesg/react-design-system/theme` instead.
...
| `default.css` import fails / not found                                    | `default.css` does not exist in v4 — each theme CSS is self-contained. Import only `theme/styles/{theme}.css`                                                                                                                                                      |
| `Layout.Container` has no horizontal padding / content flush to left edge | CSS reset uses `* { padding: 0 }` — this is unlayered and overrides `@layer fds` component padding. Use only `body { margin: 0 }`. Never put `padding`/`margin` resets on `*`, `ul`, `li`, `h1`–`h6`, `p`, or any other element selector. See setup.md reset block |
| Navbar items squished together / no space between nav links               | Same cause as above — `li { margin: 0 }` in an unlayered reset wipes the Navbar's `@layer fds` item margins. Use the minimal reset in setup.md                                                                                                                     |
| Footer doesn't stick to bottom on short pages                             | v4 `ThemeProvider` renders a wrapper `<div>` as the only child of `#root`, breaking the flex column. Add `#root > div { display: flex; flex-direction: column; flex: 1 }` to `index.css` (or `body > div` for Next.js)                                             |
