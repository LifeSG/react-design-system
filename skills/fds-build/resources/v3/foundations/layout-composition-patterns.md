...
Cross-cutting rules for assembling FDS components into polished layouts. Apply alongside per-page pattern and template files.
...
Page title pattern: `Typography.HeadingXL` as `h1`, `Typography.BodyMD` in `Colour["text-subtle"]` directly below as subtitle.
