import { Canvas, Meta, Story } from "@storybook/addon-docs";
import { DataTable } from "src/data-table";
import {
    Heading3,
    Secondary,
    StoryContainer,
    Title,
} from "../storybook-common";
import { PropsTable } from "./props-table";
import { useState } from "react";

<Meta title="General/DataTable" component={DataTable} />

<Title>Data Table</Title>

<Secondary>Overview</Secondary>

A call to action component with an icon in it.

```tsx
import { DataTable } from "@lifesg/react-design-system/data-table";
```

<Canvas>
    <Story name="DataTable">
        <DataTable
            headers={[
                {
                    colId: "status",
                    label: "Status",
                    style: { width: "7.5rem" },
                },
                {
                    colId: "time",
                    label: "Time",
                    style: { width: "15rem" },
                },
                {
                    colId: "resource",
                    label: "Resource",
                    style: { width: "17.5rem" },
                },
                { colId: "title", label: "Title" },
            ]}
            rows={[
                {
                    id: "status",
                    status: "Completed",
                    time: "07/Aug/2023 9.30pm",
                    resource: "Test",
                    title: "Titile 1",
                    colId: "status",
                    label: "Status",
                },
                {
                    id: "bookingTime",
                    status: "Pending",
                    time: "07/Aug/2023 10.30pm",
                    resource: "Test",
                    title: "Titile 2",
                    colId: "bookingTime",
                    label: "Booking time",
                },
            ]}
            isLoadingData={false}
        />
    </Story>
</Canvas>

<Heading3>Sorting</Heading3>

Table Row Sorting.

<Canvas>
    <Story name="Sorting">
        <DataTable
            headers={[
                {
                    colId: "status",
                    label: "Status",
                    style: { width: "7.5rem" },
                },
                {
                    colId: "time",
                    label: "Time",
                    style: { width: "15rem" },
                },
                {
                    colId: "resource",
                    label: "Resource",
                    style: { width: "17.5rem" },
                },
                { colId: "title", label: "Title" },
            ]}
            rows={[
                {
                    id: "status",
                    status: "Completed",
                    time: "07/Aug/2023 9.30pm",
                    resource: "Test",
                    title: "Titile 1",
                    colId: "status",
                    label: "Status",
                },
                {
                    id: "bookingTime",
                    status: "Pending",
                    time: "07/Aug/2023 10.30pm",
                    resource: "Test",
                    title: "Titile 2",
                    colId: "bookingTime",
                    label: "Booking time",
                },
            ]}
            isLoadingData={false}
            sortIndicators={{ colId: "ASC" }}
        />
    </Story>
</Canvas>

<Heading3>Checkbox selection</Heading3>

Table Row with checkbox selection.

<Canvas>
    <Story name="Checkbox selection">
        {() => {
            const [selected, setSelected] = useState();
            return (
                <DataTable
                    headers={[
                        {
                            colId: "status",
                            label: "Status",
                            style: { width: "7.5rem" },
                        },
                        {
                            colId: "time",
                            label: "Time",
                            style: { width: "15rem" },
                        },
                        {
                            colId: "resource",
                            label: "Resource",
                            style: { width: "17.5rem" },
                        },
                        { colId: "title", label: "Title" },
                    ]}
                    rows={[
                        {
                            id: "status",
                            status: "Completed",
                            time: "07/Aug/2023 9.30pm",
                            resource: "Test",
                            title: "Titile 1",
                            colId: "status",
                            label: "Status",
                        },
                        {
                            id: "bookingTime",
                            status: "Pending",
                            time: "07/Aug/2023 10.30pm",
                            resource: "Test",
                            title: "Titile 2",
                            colId: "bookingTime",
                            label: "Booking time",
                        },
                        {
                            id: "commonRow",
                            status: "Common",
                            time: "07/Aug/2023 11.30pm",
                            resource: "Test",
                            title: "Titile 3",
                            colId: "commonRow",
                            label: "Common Row",
                        },
                    ]}
                    selection={selected}
                    isLoadingData={false}
                    sortIndicators={{ colId: "ASC" }}
                    selectionConfig={{
                        showCheckboxes: true,
                        showHeaderCheckbox: false,
                        headerWidth: "5rem",
                        onClickSelect: { setSelected },
                        onClickSelectAll: { setSelected },
                    }}
                />
            );
        }}
    </Story>
</Canvas>

<Heading3>Alternating Rows</Heading3>

Table Alternating Row.

<Canvas>
    <Story name="Alternating Rows">
        {() => {
            const [selected, setSelected] = useState();
            return (
                <DataTable
                    headers={[
                        {
                            colId: "status",
                            label: "Status",
                            style: { width: "7.5rem" },
                        },
                        {
                            colId: "time",
                            label: "Time",
                            style: { width: "15rem" },
                        },
                        {
                            colId: "resource",
                            label: "Resource",
                            style: { width: "17.5rem" },
                        },
                        { colId: "title", label: "Title" },
                    ]}
                    rows={[
                        {
                            id: "status",
                            status: "Completed",
                            time: "07/Aug/2023 9.30pm",
                            resource: "Test",
                            title: "Titile 1",
                            colId: "status",
                            label: "Status",
                        },
                        {
                            id: "bookingTime",
                            status: "Pending",
                            time: "07/Aug/2023 10.30pm",
                            resource: "Test",
                            title: "Titile 2",
                            colId: "bookingTime",
                            label: "Booking time",
                        },
                        {
                            id: "commonRow",
                            status: "Common",
                            time: "07/Aug/2023 11.30pm",
                            resource: "Test",
                            title: "Titile 3",
                            colId: "commonRow",
                            label: "Common Row",
                        },
                        {
                            id: "sampleRow",
                            status: "SampleRow",
                            time: "07/Aug/2023 12.30pm",
                            resource: "Test",
                            title: "Titile 4",
                            colId: "sampleRow",
                            label: "sample Row",
                        },
                        {
                            id: "checkRow",
                            status: "CheckRow",
                            time: "07/Aug/2023 1.30pm",
                            resource: "Test",
                            title: "Titile 5",
                            colId: "checkRow",
                            label: "check Row",
                        },
                    ]}
                    isLoadingData={false}
                    sortIndicators={{ colId: "ASC" }}
                    alternatingRows={true}
                />
            );
        }}
    </Story>
</Canvas>

<Heading3>Action column</Heading3>

Table Column with Action selection.

<Canvas>
    <Story name="Action column">
        <DataTable
            headers={[
                {
                    colId: "status",
                    label: "Status",
                    style: { width: "7.5rem" },
                },
                {
                    colId: "time",
                    label: "Time",
                    style: { width: "15rem" },
                },
                {
                    colId: "resource",
                    label: "Resource",
                    style: { width: "10rem" },
                },
                { colId: "title", label: "Title" },
            ]}
            rows={[
                {
                    id: "status",
                    status: "Completed",
                    time: "07/Aug/2023 9.30pm",
                    resource: "Test",
                    title: "Titile 1",
                    colId: "status",
                    label: "Status",
                },
                {
                    id: "bookingTime",
                    status: "Pending",
                    time: "07/Aug/2023 10.30pm",
                    resource: "Test",
                    title: "Titile 2",
                    colId: "bookingTime",
                    label: "Booking time",
                },
            ]}
            isLoadingData={false}
            sortIndicators={{ colId: "ASC" }}
            actionsConfig={{
                showActions: true,
                className: "TestActionRowClass",
                headerClassName: "TestActionHeaderClass",
                headerLabel: "Actions",
                headerWidth: "2rem",
            }}
        />
    </Story>
</Canvas>

<Heading3>Empty Data View</Heading3>

Table Empty Data View.

<Canvas>
    <Story name="Empty Data View">
        <DataTable
            headers={[
                {
                    colId: "status",
                    label: "Status",
                    style: { width: "15rem" },
                },
                {
                    colId: "time",
                    label: "Time",
                    style: { width: "15rem" },
                },
                {
                    colId: "resource",
                    label: "Resource",
                    style: { width: "15rem" },
                },
                { colId: "title", label: "Title" },
            ]}
            rows={[]}
            isLoadingData={false}
            sortIndicators={{ colId: "ASC" }}
            selectionConfig={{
                showCheckboxes: true,
                showHeaderCheckbox: true,
                headerWidth: "5rem",
            }}
        />
    </Story>
</Canvas>

<Secondary>Component API</Secondary>

<PropsTable />
