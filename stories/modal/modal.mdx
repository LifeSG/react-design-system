import { Canvas, Meta } from "@storybook/addon-docs/blocks";
import { DocAlert } from "stories/storybook-common";
import * as ModalStories from "./modal.stories";
import { AutoArgTypesTabs } from "../storybook-common";
import { StyleTokensTable } from "./style-tokens-table";

<Meta of={ModalStories} />

# Modal

<DocAlert>
    **This component is deprecated.** Use
    [ModalV2](/docs/overlays-modalv2--docs) instead, which provides a more
    composable API, improved scroll behaviour, and proper accessibility.
</DocAlert>

## Overview

A window or pop up that displays over other page contents and provides information or actions
that a user can perform.

```tsx
import { Modal } from "@lifesg/react-design-system/modal";
```

There are 2 components that can be used:

-   `Modal` represents the base modal overlay. This is a **mandatory component** to be used.
-   `Modal.Box` represents the dialog box that comes with a rounded border and a close button.

<Canvas of={ModalStories.Default} />

## Custom content

<Canvas of={ModalStories.CustomContent} />

## Stacked modals

In some cases, you would require a stacked modal layout. Here is how you can construct it.

<Canvas of={ModalStories.StackedModals} />

## Modal.Box

This component holds the dialog box.

You're recommended to apply this spacing around your content to avoid overlap with the close button:

| Viewport       | Vertical | Horizontal |
| -------------- | -------- | ---------- |
| Desktop/tablet | 4rem     | 4rem       |
| Mobile         | 4rem     | 1.25rem    |

> **Note**: This component does not come with a scrollable container and it is the
> onus on the child element to possess the scroll behaviour

```tsx
<Modal.Box onClose={closeModal}>
    <ScrollableContent>
        <span>I am a Modal</span>
    </ScrollableContent>
</Modal.Box>
```

## Component API

<AutoArgTypesTabs of={ModalStories} />

## Component style tokens

<StyleTokensTable />
