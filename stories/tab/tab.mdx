...
The Tab component displays a gradient fade effect on both sides of the tab link container
when horizontal scrolling is enabled (usually on mobile devices). You can customize the fade color to match your design
by using the `fadeColor` prop.

-   `string[]` : Both left and right ends use the same fade color.

<Canvas of={TabStories.SameFadeColor} />

-   `FadeColorSet` : An object with `left` and `right` keys of type `string[]`. Left and right ends use the according fade color.
