...
`emptyContentMessage` can be passed in to control what description to show when
...
`initialScrollTime` can be specified to control the initial offset of a
horizontally scrollable timetable.

<Canvas of={TimeTableStories.InitialScrollTime} />

## Custom cell colors

You can customize the appearance of cells using the `cellStyleAttributes` prop.

<Canvas of={TimeTableStories.CustomCellColors} />

## Component API

<AutoArgTypesTabs of={TimeTableStories} />
