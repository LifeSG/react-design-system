...
type Actions<State, Reducers extends ReducerObj<State>> = {
    [T in keyof Reducers]: Reducers[T] extends (state: State) => State ? () => void : Reducers[T] extends (state: State, payload: Parameters<Reducers[T]>[1]) => State ? (...args: RequiredKeys<Parameters<Reducers[T]>[1]> extends never ? [payload?: Parameters<Reducers[T]>[1]] : [payload: Parameters<Reducers[T]>[1]]) => void : never;
};
 ...
 *
 */
export declare const useStateActions: <State, Reducers extends ReducerObj<State>>({ initialState, reducers, name, debug, }: StateActions<State, Reducers>) => [State, Actions<State, Reducers>];
export {};
